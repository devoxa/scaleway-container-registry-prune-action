...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.parseGithubInput = void 0;
...
exports.parseGithubInput = parseGithubInput;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.run = void 0;
const parseGithubInput_1 = __nccwpck_require__(429);
const scalewayApi_1 = __nccwpck_require__(362);
async function run(input) {
    const options = (0, parseGithubInput_1.parseGithubInput)(input);
    const image = await (0, scalewayApi_1.getImage)(options.scwSecretToken, options.region, options.imageName);
    const tags = await (0, scalewayApi_1.listTags)(options.scwSecretToken, options.region, image.id);
    let count = 0;
    for (const tag of tags) {
        ...
        await (0, scalewayApi_1.deleteTag)(options.scwSecretToken, options.region, tag.id);
    }
}
exports.run = run;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.deleteTag = exports.listTags = exports.getImage = void 0;
const querystring_1 = __importDefault(__nccwpck_require__(477));
async function getImage(scwSecretToken, region, imageName) {
    const response = await fetch(`https://api.scaleway.com/registry/v1/regions/${region}/images?name=${imageName}`, { headers: { 'x-auth-token': scwSecretToken } });
...
async function listTags(scwSecretToken, region, imageId) {
    const query = querystring_1.default.encode({
        page_size: 100,
    });
    const response = await fetch(`https://api.scaleway.com/registry/v1/regions/${region}/images/${imageId}/tags?${query}`, { headers: { 'x-auth-token': scwSecretToken } });
    const json = (await response.json());
...
async function deleteTag(scwSecretToken, region, tagId) {
    const query = querystring_1.default.encode({
        force: true,
    });
    const response = await fetch(`https://api.scaleway.com/registry/v1/regions/${region}/tags/${tagId}?${query}`, { method: 'DELETE', headers: { 'x-auth-token': scwSecretToken } });
    const json = (await response.json());
    if (json.id !== tagId) {
...
(0, run_1.run)({
    scwSecretToken: (0, core_1.getInput)('scw-secret-token'),
    image: (0, core_1.getInput)('image'),
    tagPattern: (0, core_1.getInput)('tag-pattern'),
    keepLast: (0, core_1.getInput)('keep-last'),
}).catch((err) => {
